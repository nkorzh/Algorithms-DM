...
class automat {
    ...
    public:
        ...
        int id;
        bool is_final, visited, used;
        node () : id(-1), is_final(false), visited(false), used(false) {}
        node ( int id ) : id(id), is_final(false), visited(false), used(false) {}
...
//        last_symbol = c;
    ...
    void dfs (int v) {
        if (state[v]->used || state[v]->visited)
            return;
...
public:
    long long count_words() {
        for (int num_term : terminal)
            dfs(num_term);
        return find_loop(0) ? -1 : counter(0);
    }
